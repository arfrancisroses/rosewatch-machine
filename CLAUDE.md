...
- **A cut flower is not a plant, so it is not a case** (per user instruction, 2026-09-22). Rose Watch monitors unauthorized sales of rose *varieties* -- plants someone can propagate and resell -- so a bouquet, a box of stems, a vase arrangement, or preserved/dried roses carrying a chart name is out of scope. Unlike the non-rose filter above, these are **not dropped silently**: the product really is a rose, so `scripts/crawl_sites.py` keeps cut-flower matches out of `new_matches` (no case is ever created) but counts them into `cut_flower_matches` / `cut_flower_match_count`, and the daily PDF itemises them under "Cut-Flower Listings -- Matched, Not Recorded" with seller, title, matched name, status and URL. If that number ever moves, it is visible and can be looked at.
  - `is_cut_flower()` is deliberately asymmetric with `is_rose_product()`: a wrong answer here **suppresses a real finding**, so silence has to be earned. Any nursery word (`PLANT_SIGNALS`: plant, bush, bare root, own root, grafted, potted, gallon, climbing, ...) settles it as a plant; only then do the form words (`CUT_FLOWER_SIGNALS`: bouquet, stems, vase, fresh cut, arrangement, preserved, dried, petals, dozen, ...) apply; a title with no signal either way is treated as a plant and matched as normal.
  - **"Cut rose", "cut flower" and "florist" are NOT signals, on purpose.** They name a breeding *class* -- varieties bred for the florist trade -- and nurseries sell plants under exactly those words. Including them suppressed 23 existing cases on the first test: Ergonzi's "Darlington Rose-达林顿｜Netherland Cut Rose" and High Garden's "Barista German Florist Hybrid Tea Rose" are plant listings. Verified against all 282 existing cases and all 246 of the 2026-09-22 run's matches: the final filter drops none of them. If a genuine plant listing is ever suppressed, remove the offending word from `CUT_FLOWER_SIGNALS` rather than loosening the plant-wins rule.
