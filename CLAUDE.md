...
**Send it immediately, and never ask first (per user instruction, 2026-09-16).** The email is pre-authorized: do not ask whether to send it, do not wait for a reply, and do not treat a re-send of a corrected report as needing permission either. `.claude/settings.json` pre-approves the Gmail draft/send tools and the scripts and git commands the run uses, so nothing in the sequence should stall on an approval prompt.
...
- **Crawl scope is closed to the known-sites roster.** Per user instruction (2026-09-10), Rose Watch scans *only* the sites listed in `data/known_sites.json` / `dashboard/KNOWN_SITES.md` — currently 15 companies / 17 URLs:
  `ergongzy.com`, `etsy.com/shop/Ergongzi`, `myroseworld.com`, `etsy.com/shop/GcmRanch`, `highgardenroses.com`, `hillsiderosefarm.com`, `huniurosegarden.ca`, `jessiesrose.com`, `etsy.com/shop/JessiesRoseUSA`, `kateroses.com`, `kisakiplant.com`, `museroses.com`, `oneloveroseandgardens.com`, `roseexplosion.com`, `springlandflowers.com`, `your-roses.com`, `facebook.com/profile.php?id=61591662794131` (Bloomora Roses, added 2026-09-15).
