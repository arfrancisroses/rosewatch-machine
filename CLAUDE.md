...
- **Your Roses is blocked at the seller's end; still waiting, next review the week of 2026-09-28 (decided 2026-09-18, extended 2026-09-21).** From 2026-09-18 `your-roses.com` answers HTTP 403 "Your request was blocked by the security firewall" (LiteSpeed/Hostinger WAF) on every catalog path. The CONNECT succeeds, so this is the site refusing us, not our egress proxy. It served 1,656 products through its WordPress feed on 2026-09-17.
  ...
  - **2026-09-24 check:** still blocked, seven days on, and the pattern moved again -- the site root answers HTTP 403 "403 - Access Denied" once more, after answering 200 from 2026-09-21 through 2026-09-23. Confirmed on two consecutive requests. Catalog paths unchanged (403 on `/wp-json/wp/v2/product`, 404 on the Shopify feed path). So the rule has now moved in both directions, which is worth carrying into Monday's conversation: whatever is being tuned is not settling.
  - The 2026-09-24 change was put to the user, who chose again to keep waiting. No action beyond the normal daily attempt until Monday.
  - **2026-09-25 check:** root back to HTTP 200, a third flip in five days (200 from 09-21 to 09-23, 403 on 09-24, 200 again today). The catalog has never once opened in eight days: 403 on `/wp-json/wp/v2/product` and 404 on the Shopify feed path, every single run. Read the root as noise, not signal -- it has moved three times while the catalog rule has not moved at all, so it says nothing about whether the block is being tuned.
...
- **A 429 is not a coverage gap -- back off and retry.** On 2026-09-24 the 06:07 run had seven Shopify stores answer HTTP 429 (High Garden, Hillside Rose Farm, Huniu Rose Garden, Kate Roses, Muse Roses, Rose Explosion, Springland Flowers). That is the host asking us to slow down, not a block and not a security control, and the crawler had no backoff, so it recorded seven healthy catalogs as failures. `fetch()` in `scripts/crawl_sites.py` now retries a 429 with exponential backoff, honouring `Retry-After`, and the run loop pauses between sites; `--only "Site A,Site B"` re-pulls named sites without touching the rest. All seven served in full on the retry, one after a single 20-second wait. Report a rate-limited site as checked **only** once it actually serves; if it never does, it stays a coverage gap like any other.
