...
def generate_cut_roses_md(cases):
    """Listings that matched a chart name but are cut flowers only -- not cases."""
    rows = cases.get("cut_roses", [])
    lines = [
        "# Cut Roses",
        "",
        "Listings that matched a name in the trademark chart but read as **cut flowers only** "
        "— stems, a bouquet, an arrangement — with nothing anywhere in the listing suggesting a "
        "plant. Per user instruction (2026-09-22) these are **not cases**: Rose Watch monitors "
        "unauthorized sales of rose *varieties*, meaning plants that can be propagated and resold.",
        "",
        "They are recorded here rather than discarded, so the number stays visible. A listing that "
        "says \"cut rose\" **and** anything plant-like — plant, bush, bare root, own root, grafted, "
        "shrub, potted, or growing language — stays an ordinary case, because \"cut rose\" is "
        "overwhelmingly a *variety class* (roses bred for the florist trade) rather than a product "
        "form. The whole listing is read, not just the title.",
        "",
        "| Seller | Product Title | Matched Trademark | TM Status | First Found | Last Verified | Product URL |",
        "|---|---|---|---|---|---|---|",
    ]
    if not rows:
        lines.append("| _(none)_ | | | | | | |")
        lines.append("")
        lines.append("No listing on the roster has been classified cut-roses-only. On the day this rule "
                     "was added, all 321 cases on record and all of Ergonzi's 504, High Garden's 670 and "
                     "Mola Rose's 754 products were checked against it, and none qualified.")
    else:
        for r in rows:
            lines.append(
                f"| {md_escape(r.get('seller_name'))} | {md_escape(r.get('exact_product_title'))} | "
                f"{md_escape(r.get('matched_trademark'))} | {md_escape(r.get('trademark_status'))} | "
                f"{md_escape(r.get('first_date_found'))} | {md_escape(r.get('last_verified'))} | "
                f"{md_escape(r.get('product_url'))} |")
    lines.append("")
    return "\n".join(lines) + "\n"
...
def generate_overview_md(trademarks, sites, cases):
    ...
    lines.append("- [Cut Roses](CUT_ROSES.md)")
...
def main():
    ...
    write(DASHBOARD_DIR / "CUT_ROSES.md", generate_cut_roses_md(cases))
